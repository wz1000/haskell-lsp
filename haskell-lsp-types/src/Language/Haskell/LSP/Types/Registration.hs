...
regHelper :: Registration m
       -> (Show (RegistrationOptions m) => ToJSON (RegistrationOptions m) => FromJSON (RegistrationOptions m) => x) -> x
regHelper (Registration _ SWorkspaceDidChangeWorkspaceFolders _) x = x
regHelper (Registration _ SWorkspaceDidChangeWatchedFiles _) x = x

-- regHelper' :: SClientMethod m
--        -> (Show (RegistrationOptions m) => ToJSON (RegistrationOptions m) => FromJSON (RegistrationOptions m) => x) -> x
-- regHelper' SWorkspaceDidChangeWorkspaceFolders x = x
-- regHelper' SWorkspaceDidChangeWatchedFiles x = x
-- regHelper' m x = x -- error at x

{-
/Users/luke/Source/haskell-lsp/haskell-lsp-types/src/Language/Haskell/LSP/Types/Registration.hs:150:18: error:
    • No instance for (Show (RegistrationOptions m))
        arising from a use of ‘x’
    • In the expression: x
      In an equation for ‘regHelper'’: regHelper' m x = x
-}

instance ToJSON (Registration m) where
  toJSON x = regHelper x (genericToJSON lspOptions x)
...
instance FromJSON SomeRegistration where
  parseJSON = withObject "Registration" $ \o -> do
    ...
    let regOptsField = "registerOptions" :: Text
    opts <-
      case m of
        SWorkspaceDidChangeWorkspaceFolders -> o .: regOptsField
        SWorkspaceDidChangeWatchedFiles     -> o .: regOptsField
        _                                   -> mempty
    r <- Registration <$> o .: "id" <*> pure m <*> pure opts
...
instance Show SomeRegistration where
  show (SomeRegistration r)= regHelper r (show r)
